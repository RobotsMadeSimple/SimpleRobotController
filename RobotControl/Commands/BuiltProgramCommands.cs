...
internal sealed class BuiltProgramCommands
{
    ...
    /// <summary>Whether the foreground executor holds <paramref name="name"/>'s variables (running, paused or just finished).</summary>
    private bool HoldsProgram(string? name) => !string.IsNullOrEmpty(name) && IsForeground(name!);
    ...
    private object? EvaluateExpression(CommandMessage msg)
    {
        ...
        {
            double value;
            if (HoldsProgram(programName))
                value = _executor!.EvaluateLive(expr);
            else
            {
    ...
    private object? GetExpressionSymbols(CommandMessage msg)
    {
        ...
        if (program != null)
        {
            var live = HoldsProgram(program.Name) ? _executor!.SnapshotLiveValues() : null;
