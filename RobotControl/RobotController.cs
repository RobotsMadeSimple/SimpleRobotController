...
namespace Controller.RobotControl
{
    internal class RobotController
    {
        ...
        // Last motion error (e.g. a MoveL/MoveJ to a point that doesn't exist) that
        // the motion thread dropped. Latched on the motion thread, consumed by the
        // program executor so a dropped move fails the step instead of passing silently.
        private string? _motionError;
        private void LatchMotionError(string message) => Volatile.Write(ref _motionError, message);
        /// <summary>Returns and clears the last latched motion error, or null if none.</summary>
        public string? ConsumeMotionError() => Interlocked.Exchange(ref _motionError, null);
